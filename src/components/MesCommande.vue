        ...
        <div class="md-layout-item md-medium-size-100 md-xsmall-size-100 md-size-100">
          <div class="md-card md-theme-default">
            <div class="md-card-header" data-background-color="grey">
              <h4 class="title">Mes commandes</h4>
                    ...
                    <table>
                      <thead>
                        <tr>
                          ...
                          <th class="md-table-head">
                            <div class="md-table-head-container md-ripple md-disabled">
                              <div class="md-table-head-label">Nom</div>
                            </div>
                          </th>
                          <th class="md-table-head">
                            <div class="md-table-head-container md-ripple md-disabled">
                              <div class="md-table-head-label">Prix Unitaire</div>
                            </div>
                          </th>
                          <th class="md-table-head">
                            <div class="md-table-head-container md-ripple md-disabled">
                      ...
                      <tbody>
                        <tr
                          class="md-table-row"
                          v-for="commande of listCommandes" :key="commande['.key']"
                        >
                          <!---->
                          <td class="md-table-cell">
                            <div class="md-table-cell-container" >
                              <img v-bind:src="commande.image">
                            </div>
                          </td>
                          <td class="md-table-cell">
...
export default {
    ...
    firebase: {
      listPanierFirebase : db.ref('panier')
    },
    mounted() {
        this.listPanierFront = [];
        this.listPanierFront = this.listPanierFirebase;
        var loggedUser = window.sessionStorage.getItem("loggedUser");
        if(loggedUser!=null)
        {
            var localList = [];
            if(this.listPanierFront == null || this.listPanierFront.length == 0)
            {
                localList = this.listPanierFirebase;
                if(localList == null || localList.length == 0)
                {
                    localList = [];
                    this.listPanierFirebase = [];
                    this.listPanierFirebase = db.ref('panier');
                    localList = this.listPanierFirebase;
                }
            }
      ...
      listCommandes: {
        get: function() {
            ...
            if(this.listPanierFront == null || this.listPanierFront.length == 0)
            {
                ...
                if(loggedUser!=null)
                {
                    for(var obj of this.listPanierFront)
                    {
                        var hashedUserId = SHA256(loggedUser);
                        if(obj.idutilisateur == hashedUserId)
                        {
                            result.push(obj);
                        }
                    }
